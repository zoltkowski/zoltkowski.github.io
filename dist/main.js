...
function showImportFeedback(success) {
...
function createTangentConstruction(pointIdx, circleIdx) {
    ...
    if (Math.abs(distToCenter - radius) < ON_CIRCLE_TOLERANCE) {
        ...
        const radiusLineStyle = { ...currentStrokeStyle(), hidden: true };
        ...
        const intersections = circleCircleIntersections({ x: midX, y: midY }, auxRadius, { x: center.x, y: center.y }, radius);
        if (intersections.length > 0) {
            // Create intersection points (tangent points on the original circle)
            const tangentPointIndices = [];
            ...
            if (tangentPointIndices.length > 0) {
                const firstLineIdx = model.lines.findIndex(l => l.points.includes(pointIdx) && l.points.includes(tangentPointIndices[0]));
                if (firstLineIdx >= 0) {
